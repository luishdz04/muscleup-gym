...
`,
		...
		try {
			let statsQuery = supabase.from('expenses').select('amount, expense_type, status');
			if (status && status !== 'all') {
				statsQuery = statsQuery.eq('status', status);
			} else if (!status) {
				statsQuery = statsQuery.neq('status', 'deleted');
			}
			...
			console.error('Error consultando estadísticas (no crítico):', statsError);
		}
