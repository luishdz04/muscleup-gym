...
export default function LayawayDialog({ 
    ...
    // ✅ 2. CARGAR COMISIONES DINÁMICAS OPTIMIZADO
    ...
    // ✅ 3. CALCULAR COMISIÓN OPTIMIZADO
    ...
    // ✅ 4. OBTENER COMISIÓN POR DEFECTO OPTIMIZADO
    ...
    // ✅ 5. MANEJAR PAGOS MIXTOS OPTIMIZADO
    ...
    }, [getDefaultCommissionRate, paymentDetails.length]);
    ...
    // ✅ 6. CALCULAR MONTOS OPTIMIZADO - SIN RE-RENDERS INFINITOS
    const calculatedValues = useMemo(() => {
      const newDepositAmount = formData.customDeposit ? 
        formData.depositAmount : 
        (totals.total * formData.depositPercentage / 100);
      
      const newRemainingAmount = totals.total - newDepositAmount;
      ...
      if (isMixedPayment) {
        ...
        newFinalAmount = paymentDetails.reduce((sum, detail) => sum + detail.amount + detail.commission_amount, 0);
      ...
      return {
        calculatedDepositAmount: newDepositAmount,
        remainingAmount: newRemainingAmount,
        commissionAmount: newCommission,
        finalDepositAmount: newFinalAmount,
    ...
    // ✅ 7. ACTUALIZAR ESTADOS CALCULADOS SIN RE-RENDERS
    useEffect(() => {
      setCalculatedDepositAmount(calculatedValues.calculatedDepositAmount);
      setRemainingAmount(calculatedValues.remainingAmount);
      setCommissionAmount(calculatedValues.commissionAmount);
      setFinalDepositAmount(calculatedValues.finalDepositAmount);
   ...
   // ✅ 8. CALCULAR FECHA DE EXPIRACIÓN
    ...
    }, [customer, calculatedDepositAmount, totals.total, formData.expirationDate, isMixedPayment, paymentDetails, formData.paymentMethod, formData.paymentReference, cashReceived, finalDepositAmount]);

    // ✅ 10. GENERAR NÚMERO DE APARTADO
    ...
    // ✅ 11. PROCESAR APARTADO CORREGIDO
    const processLayaway = useCallback(async () => {
      ...
      try {
        ...
        // ✅ CREAR VENTA TIPO LAYAWAY CON CAMPOS CORREGIDOS
        const layawayData = {
        ...
        // ✅ CREAR DETALLES DE PAGOS - USAR TABLA CORRECTA
        if (isMixedPayment) {
      ...
      validateLayaway, supabase, generateLayawayNumber, customer, formData, 
      calculatedDepositAmount, totals, isMixedPayment, paymentDetails, 
      commissionAmount, changeAmount, calculateCommission, customCommissionRate, 
      cart, coupon, cashReceived
    ]);
    ...
    // ✅ 13. CARGAR DATOS INICIALES OPTIMIZADO
    useEffect(() => {
      if (open) {
    ...
    }, [activeStep, calculatedDepositAmount, formData.expirationDate, isMixedPayment, paymentDetails.length, formData.paymentMethod, validateLayaway]);
